...
class AlumnoView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user.is_valid():
            ...
            existing_user = User.objects.filter(email=email).first()
            if existing_user:
                return Response({"message": f"El correo {email} ya está registrado."}, status=400)
            ...
            if fecha_nacimiento:
                try:
                    ...
                    return Response({"message": "Formato de fecha incorrecto. Debe ser YYYY-MM-DD."}, status=400)

            # Guardar datos adicionales en la tabla `Alumnos`
            alumno = Alumnos.objects.create(
                user=user,
                matricula=request.data.get("matricula", None),
                fecha_nacimiento=fecha_nacimiento,  # Ahora con formato validado
                ...
                ocupacion=request.data.get("ocupacion", None)
            )
            alumno.save()
